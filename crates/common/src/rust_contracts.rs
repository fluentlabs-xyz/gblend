...
impl RustContractsRegistry {
    ...
    pub fn new(contract_dir: &Path, project_root: Option<&Path>) -> Result<Self> {
        ...
        let gitignore = project_root
            .and_then(|root| load_gitignore(root).ok())
            .unwrap_or_default();
                ...
                if let Some(project_dir) = path.parent() {
                    match Self::read_project_info(project_dir) {
                        ...
                        Err(e) => {
                            ...
                            eprintln!("Warning: Failed to read Cargo.toml at {:?}: {}", path, e);
                        }
                    }
                }
            }
...
"#;
        ...
        assert_eq!(
            RustContractsRegistry::extract_sdk_version(&value),
...
"#,
    ...
    fn test_should_skip_directory() {
        let gitignore = GitignoreRules {
            patterns: vec!["target/".to_string(), "*.log".to_string()],
        };
