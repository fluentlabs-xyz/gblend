...
const RUST_GREETING_CONTRACT_CODE: &str = r#"
#![cfg_attr(not(feature = "std"), no_std, no_main)]
...
use fluentbase_sdk::{
    SharedAPI, basic_entrypoint,
    derive::{Contract, router},
};
...
#[router(mode = "solidity")]
impl<SDK: SharedAPI> GreetingApi for Greeting<SDK> {
    fn greet(&self) -> String {
...
const RUST_GREETING_CARGO_TOML: &str = r#"
[package]
name = "greeting"
version = "0.1.0"
edition = "2021"
...
const SOLIDITY_GREETING_TEST_CODE: &str = r#"
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;

import "forge-std/Test.sol";
import "../src/greeting/interface.sol";

contract GreetingTest is Test {
    function test_CallRustGreeting() public {
        Greeting greeting = new Greeting();
        string memory result = greeting.greet();
        assertEq(result, "Hello, World");
    }
}
"#;

struct GreetingProjectTestData {
    base: ForgeTestData,
}

impl GreetingProjectTestData {
    fn new() -> Self {
        let base = ForgeTestData::new(ForgeTestProfile::Default);
        let root = base.project.root();

        let rust_contract_dir = root.join("src/greeting");
        fs::create_dir_all(rust_contract_dir.join("src")).unwrap();

        fs::write(rust_contract_dir.join("Cargo.toml"), RUST_GREETING_CARGO_TOML).unwrap();
        fs::write(rust_contract_dir.join("src/lib.rs"), RUST_GREETING_CONTRACT_CODE).unwrap();

        let test_contract_path = root.join("test/GreetingTest.t.sol");
        fs::write(&test_contract_path, SOLIDITY_GREETING_TEST_CODE).unwrap();
...
static TEST_DATA_GREETING: LazyLock<GreetingProjectTestData> =
    LazyLock::new(GreetingProjectTestData::new);

#[tokio::test(flavor = "multi_thread")]
async fn test_can_compile_and_test_rust_greeting_contract_from_solidity() {
    let mut runner = TEST_DATA_GREETING.runner();
    let filter = Filter::new(".*", "GreetingTest", ".*GreetingTest.t.sol");
    let results = runner.test_collect(&filter).expect("Test collection failed");

    assert_multiple(
        &results,
        BTreeMap::from([(
            "default/test/GreetingTest.t.sol:GreetingTest",
            vec![("test_CallRustGreeting()", true, None, None, None)],
        )]),
    );
}
